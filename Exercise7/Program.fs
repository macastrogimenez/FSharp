...
let sumIt (m,n)=
    if n > 0 
    then List.fold (fun acc n -> acc + m+n) m [1.. +1 .. n]
    else m 
...
// Alternative functional iterative declaration using tail recursion with accumulator
let sumAcc (m,n) =
    let rec sumHelper i acc =
        if i > n then acc
        else sumHelper (i + 1) (acc + m + i)
    
    if n <= 0 then m
    else sumHelper 0 0

// test: sumAcc (4,1);; -> should be 9
// test: sumAcc (5,1);; -> should be 11  
// test: sumAcc (5,10);; -> should be 105
